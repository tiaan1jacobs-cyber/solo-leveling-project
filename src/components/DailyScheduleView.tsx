      ...
      {selectedBlock && (
        ...
        >
          <motion.div
            ...
            style={{
              background: soloLevelingTheme.background.card,
              border: `2px solid ${getTypeColor(selectedBlock.type)}`,
              boxShadow: `0 0 40px ${getTypeColor(selectedBlock.type)}40`,
            }}
          ...
          >
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div style={{ color: getTypeColor(selectedBlock.type) }}>
                    {getTypeIcon(selectedBlock.type)}
                  </div>
                  <div>
                    <h2 className="text-3xl font-bold text-white">{selectedBlock.activity}</h2>
                    <p className="text-slate-400 font-mono">
                      {selectedBlock.time} - {selectedBlock.endTime} ({calculateDuration(selectedBlock.time, selectedBlock.endTime)})
                    </p>
                  </div>
                </div>
                <button
              ...
              {selectedBlock.description && (
                <p className="text-slate-300 mb-4">{selectedBlock.description}</p>
              )}

              {/* Tags */}
              <div className="flex flex-wrap gap-2">
                {selectedBlock.affirmationCategory && (
                  <span
                    className="px-3 py-1 rounded-full text-sm font-medium"
                    style={{
                      background: `${soloLevelingTheme.glow.secondary}30`,
                      color: soloLevelingTheme.glow.secondary,
                    }}
                  >
                ...
                {selectedBlock.combatType && (
                  <span
                    className="px-3 py-1 rounded-full text-sm font-medium"
                    style={{
                      background: `${soloLevelingTheme.stats.strength}30`,
                ...
                <span
                  className="px-3 py-1 rounded-full text-sm font-medium"
                  style={{
                    background: `${soloLevelingTheme.glow.primary}30`,
                    color: soloLevelingTheme.glow.primary,
                  }}
                >
                  <Zap className="w-4 h-4 inline mr-1" />
                  +{selectedBlock.xpReward} XP
                </span>
              </div>
            </div>
            ...
            <div className="mb-6">
              <h3 className="text-xl font-bold text-white mb-4 flex items-center justify-between">
                ...
                {isEditing && (
                  <button
                    onClick={() => {
                      const newChecklist = [...(editedBlock?.checklist || []), ''];
                      setEditedBlock({...editedBlock!, checklist: newChecklist});
                    }}
              ...
              <div className="space-y-2">
                {isEditing ? (
                  (editedBlock?.checklist || getTaskDetails(selectedBlock).checklist).map((item, index) => (
                    <div key={index} className="flex items-start gap-2">
                      <input
                        ...
                        className="flex-1 p-3 rounded-lg text-slate-300 text-sm"
                        style={{
                          background: soloLevelingTheme.background.secondary,
                          border: `1px solid ${soloLevelingTheme.border.default}`,
                        }}
                      ...
                      <button
                        onClick={() => {
                          const newChecklist = (editedBlock?.checklist || []).filter((_, i) => i !== index);
                          setEditedBlock({...editedBlock!, checklist: newChecklist});
                        }}
                        className="p-3 rounded-lg text-red-400 hover:text-red-300"
                        style={{
                          background: soloLevelingTheme.background.secondary,
                          border: `1px solid ${soloLevelingTheme.border.default}`,
                        }}
                  ...
                  getTaskDetails(selectedBlock).checklist.map((item, index) => (
                    <motion.div
                      key={index}
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
            ...
            <div className="mb-6">
              <h3 className="text-xl font-bold text-white mb-4 flex items-center justify-between">
                ...
                {isEditing && (
                  <button
                    onClick={() => {
                      const newNotes = [...(editedBlock?.notes || []), ''];
                      setEditedBlock({...editedBlock!, notes: newNotes});
                    }}
                    className="text-sm text-blue-400 hover:text-blue-300 flex items-center gap-1"
                  >
                    <Plus className="w-4 h-4" />
              ...
              <div className="space-y-2">
                {isEditing ? (
                  (editedBlock?.notes || getTaskDetails(selectedBlock).notes).map((note, index) => (
                    <div key={index} className="flex items-start gap-2">
                      <input
                        type="text"
                        value={note}
                        ...
                        className="flex-1 p-3 rounded-lg text-slate-300 text-sm"
                        style={{
                          background: `${soloLevelingTheme.stats.warning}15`,
                          border: `1px solid ${soloLevelingTheme.stats.warning}40`,
                        }}
                        placeholder="Enter note..."
                      />
                      <button
                        onClick={() => {
                          const newNotes = (editedBlock?.notes || []).filter((_, i) => i !== index);
                          setEditedBlock({...editedBlock!, notes: newNotes});
                        }}
                        className="p-3 rounded-lg text-red-400 hover:text-red-300"
                        style={{
                  ...
                  getTaskDetails(selectedBlock).notes.map((note, index) => (
                    <motion.div
                      key={index}
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
              ...
              {!isEditing ? (
                <>
                  <motion.button
                    ...
                    className="px-6 py-3 rounded-xl font-bold text-blue-400 hover:text-white transition-all"
                    style={{
                      background: soloLevelingTheme.background.secondary,
                      border: `1px solid ${soloLevelingTheme.border.default}`,
                    }}
                  ...
                  <motion.button
                    ...
                    style={{
                      background: completedBlocks.has(selectedBlock.id)
                        ? soloLevelingTheme.background.secondary
                        : `linear-gradient(135deg, ${soloLevelingTheme.glow.primary}, ${soloLevelingTheme.glow.secondary})`,
                      border: `1px solid ${completedBlocks.has(selectedBlock.id) ? soloLevelingTheme.border.default : soloLevelingTheme.glow.primary}`,
                      boxShadow: completedBlocks.has(selectedBlock.id) ? 'none' : soloLevelingTheme.shadow.glow,
                    }}
                  >
                ...
                <>
                  <motion.button
                    ...
                    onClick={async () => {
                      if (editedBlock) {
                        await saveBlockChanges(editedBlock);
                        setSelectedBlock(editedBlock);
                        setIsEditing(false);
                      }
                    }}
                    className="flex-1 py-3 rounded-xl font-bold text-white transition-all"
                    style={{
                      background: `linear-gradient(135deg, ${soloLevelingTheme.glow.primary}, ${soloLevelingTheme.glow.secondary})`,
                      border: `1px solid ${soloLevelingTheme.glow.primary}`,
